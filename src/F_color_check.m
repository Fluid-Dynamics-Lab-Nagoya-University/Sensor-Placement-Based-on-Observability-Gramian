function F_color_check(color_set)
% Acknowledgments:
% HUGE THANKS TO JRM, who dedicated his enormous efforts to the 
%   matlab_ipsum generator. 
% NOT FORGETTABLE TO APPRECIATE TO YA for cprintf, who reminds me of 
%   MATLAB users who are willing to share their astonishing abilities.
% 
% Usage:    F_color_check(color_set) to indicate what colors are included
%           in the color sets.
%
% Example:  just type F_color_check(jet(10)) in the command window

cprintf('-comment','↓ included colors ↓ \n')
for i = 1: size(color_set,1)
t = matlab_ipsum('Paragraphs',1,'Sentences',1);
cprintf(color_set(i,:),[sprintf('%03i ',i),sprintf('[%01.2f, %01.2f, %01.2f]',...
    color_set(i,1),color_set(i,2),color_set(i,3)),': ', t ,'\n'])
end
cprintf('-comment','↑ included colors ↑ \n')
end
